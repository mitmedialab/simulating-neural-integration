...
def main():
    ...
    for i, scenario in enumerate(scenarios, 1):
        ...
        print(f"\n[{i}/20] Generating response...")

        # Generate unsteered response
        response = generate_unsteered(model, system_prompt, action, max_length=100)

        # Score the response
        raw_score, normalized_score = score_response(model, response, trait, persona_vectors_path, device)
        ...
        print(f"Raw: {raw_score:.6f} | Normalized: {normalized_score:.6f}")

    # Save results
    output_file = Path("scripts/results/control.json")
